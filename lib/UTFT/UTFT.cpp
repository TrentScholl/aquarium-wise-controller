...
//Une partie de l'implémentation se trouve dans HW_ATmega1280.h et HW_AVR_defines.h
...
UTFT::UTFT(byte dummy)
{
  //Constructeur de cet lib modifiée - ONLY FOR SSD1289 (sainsmart, see below) WITH ARDUINO MEGA
  //Screen : SainSmart 3.2" TFT LCD Display+Touch Panel+PCB adapter SD Slot for Arduino 2560
  //Shield : SainSmart TFT LCD Adjustable Shield for Arduino Mega 2560 R3 1280 A082 Plug
  //Shield modified : corrections for SD card

  disp_x_size=239;
	disp_y_size=319;
	display_transfer_mode=16;

  _set_direction_registers(0);
	
  //Infos pins (voir les macros dans HW_AVR_defines.h)
  //38 PD7  RS 
  //39 PG2  WR
  //40 PG1  CS
  //41 PG0  RST
...
void UTFT::drawRect(int x1, int y1, int x2, int y2)
{
  ...
  Trame[1] = 10; //taille de la sous-trame (à partir de cet endroit, on compte la size mais pas l'ID termX
  Trame[2] = 0xA0; //ID ordre d'aff drawRect
  Trame[3] = x1 >> 8; //MSB x1
  Trame[4] = x1; //LSB x1
  Trame[5] = y1 >> 8;
  Trame[6] = y1; 
...
void UTFT::drawRoundRect(int x1, int y1, int x2, int y2)
{
  ...
  Trame[2] = 0xA4; //ID ordre d'aff RoundRect
  Trame[3] = x1 >> 8; //MSB x1
  Trame[4] = x1; //LSB x1
  Trame[5] = y1 >> 8;
  Trame[6] = y1; 
...
void UTFT::fillRect(int x1, int y1, int x2, int y2)
{
  ...
  Trame[0] = 0x81; //ID de Terminal X
  Trame[1] = sizeof(Trame)-1; //taille de la sous-trame (à partir de cet endroit, on compte la size mais pas l'ID termX
  Trame[2] = TX_FILLRECT; //ID ordre d'aff fillRect
  Trame[3] = x1 >> 8; //MSB x1
  Trame[4] = x1; //LSB x1
  Trame[5] = y1 >> 8;
  Trame[6] = y1; 
...
void UTFT::fillRoundRect(int x1, int y1, int x2, int y2)
{
  ...
  Trame[0] = 0x81; //ID de Terminal X
  Trame[1] = sizeof(Trame)-1; //taille de la sous-trame (à partir de cet endroit, on compte la size mais pas l'ID termX
  Trame[2] = 0xA5; //ID ordre d'aff fillRoundRect
  Trame[3] = x1 >> 8; //MSB x1
  Trame[4] = x1; //LSB x1
  Trame[5] = y1 >> 8;
  Trame[6] = y1; 
...
void UTFT::clrScr()
{
  ...
  uint8_t Trame[4];
  Trame[0] = 0x81; //ID de Terminal X
  Trame[1] = 5; //taille de la sous-trame (à partir de cet endroit, on compte la size mais pas l'ID termX
  Trame[2] = TX_CLRSCR; //ID ordre d'aff clrscr
  Trame[3] = 0;
  Serial.write(Trame, sizeof(Trame));
  //return;
#endif
...
void UTFT::setColor(byte r, byte g, byte b)
{
  ...
  Trame[0] = 0x81; //ID de Terminal X
  Trame[1] = 5; //taille de la sous-trame (à partir de cet endroit, on compte la size mais pas l'ID termX
  Trame[2] = TX_SETCOLOR; //ID ordre d'aff setColor
  Trame[3] = r;
  Trame[4] = g; //LSB x1
  Trame[5] = b;
  Serial.write(Trame, sizeof(Trame));
...
void UTFT::setBackColor(byte r, byte g, byte b)
{
  ...
  Trame[0] = 0x81; //ID de Terminal X
  Trame[1] = 5; //taille de la sous-trame (à partir de cet endroit, on compte la size mais pas l'ID termX
  Trame[2] = TX_SETBACKCOLOR; //ID ordre d'aff setColor
  Trame[3] = r;
  Trame[4] = g; //LSB x1
  Trame[5] = b;
  Serial.write(Trame, sizeof(Trame));
...
void UTFT::drawLine(int x1, int y1, int x2, int y2)
{
  ...
  Trame[0] = 0x81; //ID de Terminal X
  Trame[1] = sizeof(Trame)-1; //taille de la sous-trame (à partir de cet endroit, on compte la size mais pas l'ID termX
  Trame[2] = TX_DRAWLINE; //ID ordre d'aff line
  Trame[3] = x1 >> 8; //MSB x1
  Trame[4] = x1; //LSB x1
  Trame[5] = y1 >> 8;
  Trame[6] = y1; 
...
void UTFT::drawHLine(int x, int y, int l)
{
  ...
  Trame[0] = 0x81; //ID de Terminal X
  Trame[1] = sizeof(Trame)-1; //taille de la sous-trame (à partir de cet endroit, on compte la size mais pas l'ID termX
  Trame[2] = TX_DRAWHLINE; //ID ordre d'aff lineH
  Trame[3] = x >> 8; //MSB x1
  Trame[4] = x; //LSB x1
  Trame[5] = y >> 8;
  Trame[6] = y; 
...
void UTFT::drawVLine(int x, int y, int l)
{
  ...
  Trame[0] = 0x81; //ID de Terminal X
  Trame[1] = sizeof(Trame)-1; //taille de la sous-trame (à partir de cet endroit, on compte la size mais pas l'ID termX
  Trame[2] = TX_DRAWVLINE; //ID ordre d'aff lineV
  Trame[3] = x >> 8; //MSB x1
  Trame[4] = x; //LSB x1
  Trame[5] = y >> 8;
  Trame[6] = y; 
...
void UTFT::print(char *st, int x, int y, int deg)
{
  ...
  uint8_t Trame[40];
  Trame[0] = 0x81; //ID de Terminal X
  Trame[1] = 7 + strlen(st); //taille de la sous-trame (à partir de cet endroit, on compte la size mais pas l'ID termX
  Trame[2] = TX_PRINTCHAR; //ID ordre d'aff print char
  //TODO bug si pas de param !
  Trame[3] = x >> 8; 
  Trame[4] = x; 
  Trame[5] = y >> 8; 
  Trame[6] = y; 
  ...
  for (int i = 0; i < strlen(st); i++)
    Trame[8 + i] = st[i];
  Serial.write(Trame, 8 + strlen(st));
  //return;
#endif
...
void UTFT::lcdOn()
{
	cbiCS;
	//cbi(P_CS, B_CS);
  LCD_Write_COM_DATA(0x07,0x0021);  //display control
  LCD_Write_COM_DATA(0x00,0x0001);  //oscillator start 
  LCD_Write_COM_DATA(0x07,0x0023);
  LCD_Write_COM_DATA(0x10,0x0000);
  delay(30);
  LCD_Write_COM_DATA(0x07,0x0033);
  LCD_Write_COM_DATA(0x11,0x60B0); // ID=11  AM=0
...
//in test. Actually no effect visible !
void UTFT::Optimize_speed()
{
	cbi(P_CS, B_CS);
...
void UTFT::STANDBY_ON (void)
{
	cbi(P_CS, B_CS);
	// Display Off
	LCD_Write_COM_DATA(0x0010,0x0001); 
	sbi(P_CS, B_CS);
}

void UTFT::STANDBY_OFF (void)
{
	cbi(P_CS, B_CS);
	// Start Oscillation
...
void UTFT::Lcd_ScrollAreaSetup(int VL1, int VL2)
{  
	 //Lcd_W_com_data(0x0041,VL1);
	 //Lcd_W_com_data(0x0042,VL2);
	cbi(P_CS, B_CS);
	LCD_Write_COM_DATA(0x41, VL1);
	//sbi(P_CS, B_CS);
	//cbi(P_CS, B_CS);
	LCD_Write_COM_DATA(0x42, VL2);
	sbi(P_CS, B_CS);
}

void UTFT::Scroll_Up(void) //scroll hardware simple de l'affichage
{
	if (line <= 320)
	{
